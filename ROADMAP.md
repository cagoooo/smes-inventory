...
## 🎯 v5.0 建議優先清單（2 週內可做，高價值）

> 這些是**馬上開始用就會感覺到差異**的項目，投資報酬率最高。

### 🔴 P0（強烈建議）
| 任務 | 價值 | 難度 | 估時 |
|------|:---:|:---:|:---:|
| **Google OAuth 登入**（限定 @mail2.smes.tyc.edu.tw 網域）取代 anon 讀寫 | ★★★★★ | 中 | 3h |
| **教室平面圖可視化**（SVG 依 PDF 重繪，點教室直接選） | ★★★★★ | 中 | 4h |
| **盤點進度條**（每間教室已拍/未拍顯示百分比） | ★★★★ | 易 | 1h |
| **連續拍照模式**（拍完上一張儲存後自動回到拍照狀態） | ★★★★ | 易 | 1h |
| **QR Code 掃描**（財產標籤上有 QR code 直接掃，省 Gemini Token） | ★★★★ | 中 | 2h |
